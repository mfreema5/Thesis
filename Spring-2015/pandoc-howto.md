...
* To generate a PDF file from a markdown file:
  * `pandoc -f markdown_github --latex-engine=xelatex -V geometry:margin=0.75in -s -o "filename.pdf" "filename.md"`
* To generate LaTeX source from a markdown file:
  * `pandoc -f markdown_github --latex-engine=xelatex -V geometry:margin=0.75in -s -o "filename.tex" "filename.md"`
...
* To generate PDFs from all the markdown files in a directory:
  * `for FOO in *.md ; do pandoc -f markdown_github --latex-engine=xelatex -V geometry:margin=0.75in -s -o "${FOO%%.md}.pdf" "$FOO" ; done`
